...
class HTTPBenchmark(object):
    ...
    def main(self):
        ...
        parser.add_argument('-v', '--version', action='version',
                version='pb version {0}'.format('.'.join(str(i) for i in VERSION)),
        ...
        for x in xrange(self.args.number):
            ...
            if self.iscallable(worker):
                try:
                    ...
                    self.log.error("worker raised an exception")
                    raise
            else:
                self.log.error("get_worker didn't return a valid callable object: {0}".format(worker))
    ...
    def finish_run(self):
        ...
        for url in self.results:
            ...
            # {'redirect': 0.0, 'queue': 0.34013509750366211, 'pretransfer': 0.0096919999999999992, 'starttransfer': 0.020733000000000001, 'connect': 0.0096810000000000004, 'total': 0.020902, 'namelookup': 3.1999999999999999e-05}
